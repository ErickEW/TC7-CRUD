...
app.get("/", (req, res) => {
    ...
    (async () => {
        ...
        // Mostrar como JSON
        // console.log(JSON.stringify(animes, null, 4));

        // Mostrar con un for
        // Cada Anime tiene la siguiente estructura:
        // {
        //     name: 'Tate no Yusha',
        //     gender: 'Isekai',
        // }
        ...
        animes.forEach((anime) => {
            ...
            console.log('anime.gender', anime.gender);
            console.log('anime.year', anime.year);

        });

        // Renderizar (mostrar) el archivo ejs: /views/pages/index.ejs
        res.render("pages/index", {
            // Pasar la información de los animes al ejs (html) para mostrarlos
            // Permitir que el template utilice una variable animes: cuyo valor será la variable animes
...
app.post("/create", (req, res) => {
    ...
    (async () => {
        // Crear un nuevo Anime (registro en la base de datos) usando la
        // información obtenida desde el formulario
        let game = await Anime.create({
            name: name,
            gender: gender,
            year: year,
...
app.post('/Reviews/create', (req, res, next) => {
    ...
    (async () => {
        await Review.create({
            ...
            // animeId es una columna que se agrega automáticamente debido a la relación
            // entre ambos modelos
            animeId: animeId,
        });

        res.redirect('/detail/' + animeId);
    })();
});
